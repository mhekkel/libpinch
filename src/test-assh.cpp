...
{
  public:
			client(assh::std::shared_ptr<basic_connection> connection)
				: m_first(true)
			{
...
int main(int argc, char* const argv[])
{
	try
	{
		...
		assh::std::shared_ptr<basic_connection> connection(pool.get(user, host, boost::lexical_cast<int16_t>(port)));
