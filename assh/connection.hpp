...
class rekey_state_machine
{
  public:
	rekey_state_machine(connection2& connection);

	bool process(ipacket& in, opacket& out, boost::system::error_code& ec);

  private:
	connection2& m_connection;
	std::shared_ptr<key_exchange> m_key_exchange;
	std::vector<uint8_t> m_my_payload, m_host_payload, m_session_id;
};

class connect_state_machine
{
  public:
	connect_state_machine(connection2& connection)
		: m_connection(connection), m_state(userauth) {}

	bool process(ipacket& in, opacket& out, boost::system::error_code& ec);

	connection2& m_connection;
	enum { rekeying, rekeying2, rekeying3, newkeys,
		userauth,
		done } m_state;
};
...
class connection2 : public std::enable_shared_from_this<connection2>
{
  public:
	...
	friend class rekey_state_machine;
